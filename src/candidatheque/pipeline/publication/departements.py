...
« 08 » si le S est un 8 mal imprimé, « 05 » s'il est un 5 : le territoire lu à
côté tranche, mais une substitution appliquée sans le regarder se tromperait une
...
@cache
def _tables() -> tuple[dict[str, str], dict[str, str], dict[str, tuple[str, int]]]:
    """Les index du registre : par nom, par code courant, par code ancien."""
    ...
    return par_nom, par_code, anciens
...
def normaliser(brut: str | None, annee: int) -> str | None:
    ...
    if not brut:
        return None
    par_nom, par_code, anciens = _tables()
